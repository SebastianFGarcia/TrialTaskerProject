<template>
    <Head title="Tipo de etapa" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tipo de etapas</h2>
        </template>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                    <HeaderTable :getData="getData" :create="openModal" />
                    <Table :data="typeStage.data" :columns="columns" name="typeStage" :edit="openModal" :destroy="destroy"
                        :getData="getData" />
                    <Pagination :data="typeStage" :getData="getData" />
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
        ...
        <div class="p-6">
            ...
            <form @submit.prevent="save">
                <div class="mt-6">
                    ...
                    <TextInput id="name" ref="nameInput" v-model="form.name" type="text" class="mt-1 block w-full sm:w-3/4"
                        placeholder="Nombre" />
...
const typeStage = usePage().props.typeStage;
...
const openModal = (op, param) => {
    ...
    operacion.value = op;
    if (op == 1) {
        title.value = 'Crear tipo de etapa';
    } else {
        title.value = 'Editar tipo de etapa';
        id.value = param.id;
...
const save = () => {
    if (operacion.value == 1) {
        form.post(route('typeStage.store'), {
            preserveScroll: true,
            onSuccess: () => {
                ok('Tipo de etapa creado');
                getData();
            }
        });
    } else {
        form.patch(route('typeStage.update', id.value), {
            preserveScroll: true,
            onSuccess: () => {
                ok('Tipo de etapa actualizado');
                getData();
            }
        });
    }
...
const destroy = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios.delete(route('typeStage.destroy', id))
                .then((response) => {
                    getData();
                    Toast.fire({
                        icon: 'success',
...
function getData(sort = '', per_page = '', page = '', search = '', direction = '') {
    ...
    page = page != '' ? page : typeStage.last_page > url.searchParams.get('page') ? url.searchParams.get('page') : 1;
